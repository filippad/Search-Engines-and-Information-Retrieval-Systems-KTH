...
public class Engine {
    ...
    public Engine( String[] args ) {
         ...
         *   Access to the index is synchronized since we don'getIDF want to
        ...
        if (is_indexing) {
            synchronized ( indexLock ) {
                ...
                //gui.displayInfoText("Reading pageranks...");
                startTime = System.currentTimeMillis();
                readTitles();
                readPageRanks();
                elapsedTime = System.currentTimeMillis() - startTime;
                //gui.displayInfoText( String.format( "Reading pageranks done in %.1f seconds.", elapsedTime/1000.0 ));
                index.cleanup();

                // Print the result for "ve" and "th he"
                String kGram = "ve";
                printKgramResult(kGram);
                kGram = "th he";
                printKgramResult(kGram);
            }
        } else {
    ...
    private void readTitles () {
        try {
            BufferedReader in = new BufferedReader(new FileReader("./pagerank/davisTitles.txt"));
            String line;
            Integer title;
            String name;
            while ((line = in.readLine()) != null) {
                int ind = line.indexOf(";");
                title = Integer.parseInt(line.substring(0, ind));
                name = line.substring(ind +1).trim();
                index.titleToName.put(title, name);
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private void readPageRanks () {
        try {
            BufferedReader in = new BufferedReader(new FileReader("./pagerank/pageRanks.txt"));
            String line;
            Integer title;
            String docName;
            Integer docID;
            ...
            while ((line = in.readLine()) != null) {
                int i = line.indexOf(":");
                title = Integer.parseInt(line.substring(0, i));
                docName = index.titleToName.get(title);
                docID = index.docIDs.get(docName);
                pr = Double.parseDouble(line.substring(i+2));
                index.pageRanks.put(docID, pr);
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private void printKgramResult(String kGram) {
        String[] kgrams = kGram.split(" ");
